...
def crawl_post(url, time_to_read=0, category_id=None):
    if db.post.find_one({'url_post': url[30:]}) is not None:
        return
    ...
    user_url = driver.find_element(By.XPATH, "//h3[@class='username']").find_element_by_tag_name('a').get_attribute(
        "href")
    ...
    try:
        list_driver_hashtag = driver.find_element(By.XPATH,
                                                  "//ul[@class='tags-list list-unstyled clearfix']").find_elements_by_tag_name(
            'li')
    ...
    for driver_cmt in list_driver_cmt:
        ...
        user_url = driver_cmt.find_element(By.XPATH, ".//h3[@class='name']").find_element_by_tag_name(
...
def crawl_user(url):
    ...
    user.avatar = driver.find_element(By.XPATH, "//div[@class='avatar']").find_element_by_tag_name('img').get_attribute(
        "src").strip()
    try:
        user.cover_img = driver.find_element(By.XPATH, "//div[@class='cover']").find_element_by_tag_name(
            'img').get_attribute("src").strip()
    except:
        user.cover_img = driver.find_element(By.XPATH, "//div[@class='thumb clearfix']").find_element_by_tag_name(
            'img').get_attribute("src").strip()
    user.insert_to_db()
    driver.close()
    driver.quit()
...
def crawl_category(url):
    ...
    if category_find is None:
        ...
        category.name_category = driver.find_element(By.XPATH, "//div[@id='category-heading']").find_element(By.XPATH,
                                                                                                             ".//div[@class='title']").text.strip()
        category.url_images = driver.find_element(By.XPATH, "//div[@class='background']").find_element_by_tag_name(
            'img').get_attribute("src").strip()
    ...
    for i, driver_post in enumerate(list_driver_post):
        if i == 0:
            continue
        post_url = driver_post.find_element(By.XPATH, ".//h3[@class='title']").find_element_by_tag_name(
            'a').get_attribute("href")
        time_to_read = int(re.findall(r"\d+",
                                      re.findall(r"\d+ phút đọc",
                                                 driver_post.find_element(By.XPATH, ".//div[@class='created']").text)[
                                          0])[0])
